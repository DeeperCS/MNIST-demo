...
def crop(im):
	...
	idx = np.where(im_arr==0)
	top = idx[0].min()
	bottom = idx[0].max()
	left = idx[1].min()
	right = idx[1].max()
	print("top, bottom, left, right:", top, bottom, left, right)
	crop_threshold = int(0.1*height)
	...
	cropped_right = width
	# import pdb; pdb.set_trace()
	if top>crop_threshold:
		cropped_top = (top-crop_threshold)
	if (height-bottom)>crop_threshold:
		cropped_bottom = bottom+crop_threshold
	if left>crop_threshold:
		cropped_left = (left-crop_threshold)
	if (width-right)>crop_threshold:
		cropped_right = right+crop_threshold
	# print("cropped_top, cropped_bottom, cropped_left, cropped_right:", cropped_top, cropped_bottom, cropped_left, cropped_right)
	return Image.fromarray(im_arr[cropped_top:cropped_bottom, cropped_left:cropped_right])
...
def main_page():
	if request.method == 'POST':
		...
		# im.save('im_original.png', 'PNG')
		#### im = crop(im)
		#### im.save('im_cropped.png', 'PNG')
		# import pdb; pdb.set_trace()
		imresize = im.resize((28,28),Image.ANTIALIAS)
		# imresize.save('im_resize.png', 'PNG')
